...
function AuthGate() {
  ...
  const handleSetup = async () => {
    ...
    const pwEl = document.getElementById("setup-pw");
    const n = nameEl?.value.trim(), e = emailEl?.value.trim(), p = pwEl?.value;
    if (!n || !e || !p || p.length < 6) { setError("Enter name, email, and a password of at least 6 characters."); return; }
    setLoading(true); setError("");
    const { data, error: signupErr } = await supabase.auth.signUp({ email: e, password: p });
    if (signupErr) { setError(signupErr.message); setLoading(false); return; }
    await supabase.from("profiles").insert({ id: data.user.id, name: n, email: e, role: "owner", permissions: [] });
    // Auto sign in
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div className="wj-card" style={{ width: 380, padding: 28 }}>
        ...
        {mode === "setup" ? (
          <>
            ...
            <div style={{ fontSize: 12.5, color: "#9A9D9F", marginBottom: 18, lineHeight: 1.5 }}>
              This is the first-time setup. Create the owner login — full access to everything including analytics, costing, and staff management.
            </div>
            <div style={{ marginBottom: 12 }}><label>Your name</label><input id="setup-name" placeholder="e.g. Don" /></div>
            <div style={{ marginBottom: 12 }}><label>Email address</label><input id="setup-email" type="email" placeholder="you@example.com.au" /></div>
            <div style={{ marginBottom: 16 }}>
              <label>Password (min 6 characters)</label>
              <PasswordInput value={password} onChange={setPassword} show={showPw} setShow={setShowPw} onEnter={handleSetup} />
            </div>
        ...
        <div style={{ fontSize: 11, color: "#5C6065", marginTop: 18, lineHeight: 1.5 }}>
          Staff accounts are created by the owner from inside the app. If you need access, contact your manager.
        </div>
      </div>
    </div>
  );
...
function MainApp({ session, onLogout, clients, parts, jobs, staffUsers, persistClients, persistParts, persistJobs, persistStaff }) {
  ...
  const jobCost = useCallback((job) => {
    const partsCost = (job.partsUsed || []).reduce((sum, pu) => {
      ...
      return sum + (part ? part.cost * pu.qty : 0);
    }, 0);
  ...
  return (
    <div>
      <div style={{ borderBottom: "1px solid #2C2F33", padding: "14px 22px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 6, background: "#FF8A1E", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Wrench size={18} color="#1A1300" />
          </div>
          <div className="wj-h" style={{ fontSize: 19, fontWeight: 700 }}>Workshop Floor</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 13, fontWeight: 600 }}>{session.name}</div>
            <div style={{ fontSize: 11, color: isOwner ? "#FF8A1E" : "#9A9D9F", fontWeight: 600, textTransform: "uppercase" }}>{isOwner ? "Owner / full access" : "Staff"}</div>
          </div>
          {isOwner && (
            <button className="wj-btn wj-ghost" style={{ padding: "9px 12px", borderRadius: 6, display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600 }} onClick={() => setShowMobileInfo(true)}>
              <Smartphone size={15} /> Staff app access
            </button>
          )}
          <button className="wj-btn wj-ghost" style={{ padding: "9px 12px", borderRadius: 6, display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600 }} onClick={onLogout}>
      ...
      <div style={{ display: "flex" }}>
        <div style={{ width: 188, borderRight: "1px solid #2C2F33", padding: "16px 10px", flexShrink: 0, minHeight: "calc(100vh - 63px)" }}>
        ...
        <div style={{ flex: 1, padding: "24px 28px", minWidth: 0 }}>
          ...
          {tab === "none" && (
            <div className="wj-card" style={{ padding: 32, textAlign: "center", color: "#9A9D9F" }}>
              ...
              <div style={{ fontSize: 14 }}>No modules have been assigned to your account yet. Contact the owner to set up your access.</div>
            </div>
          )}
        </div>
      </div>
      ...
      {filtered.length === 0 ? <Empty text="No jobs match. Create a new job card to get started." /> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map((j) => {
            ...
            return (
              <div key={j.id} className="wj-card" style={{ padding: 16, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                <div style={{ flex: "2 1 220px", minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12.5, color: "#9A9D9F" }}>{clientById[j.clientId]?.name || "No client"} · {clientById[j.clientId]?.phone || ""}</div>
                  {(j.assignedTo || []).length > 0 && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 4 }}>
                      {(Array.isArray(j.assignedTo) ? j.assignedTo : [j.assignedTo]).map((name) => (
                        <span key={name} style={{ fontSize: 11, padding: "2px 7px", borderRadius: 20, background: "#2C2F33", color: "#C7C5BE", border: "1px solid #3A3D42" }}>
                          {name}
                        </span>
                      ))}
                    </div>
                ...
                <div style={{ flex: "1 1 130px" }}>
                  <select value={j.status} onChange={(e) => setStatus(j.id, e.target.value)} style={{ fontSize: 12.5, fontWeight: 600, color: STATUS_COLOR[j.status], borderColor: STATUS_COLOR[j.status] }}>
                    {STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                {isOwner && <div style={{ flex: "0 0 100px", fontSize: 13, color: "#C7C5BE" }}>{money(cost.total)}</div>}
                <div style={{ flex: "0 0 90px", fontSize: 12, color: "#9A9D9F" }}>{dateShort(j.createdAt)}</div>
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  <IconBtn onClick={() => setPrintJob(j)} title="Print job card"><Printer size={15} /></IconBtn>
                  <IconBtn onClick={() => { setEditing(j); setShowForm(true); }} title="Edit"><ChevronRight size={15} /></IconBtn>
                  {isOwner && (
                    confirmDeleteId === j.id ? (
                      <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                        <span style={{ fontSize: 11.5, color: "#D9695A", fontWeight: 600, whiteSpace: "nowrap" }}>Delete?</span>
                        <button className="wj-btn" onClick={() => removeJob(j.id)} style={{ background: "#A23B2E", color: "#fff", padding: "4px 10px", borderRadius: 5, fontSize: 12, fontWeight: 700 }}>Yes</button>
                        <button className="wj-btn wj-ghost" onClick={() => setConfirmDeleteId(null)} style={{ padding: "4px 8px", borderRadius: 5, fontSize: 12 }}>No</button>
                      </div>
                    ) : (
                      <IconBtn onClick={() => setConfirmDeleteId(j.id)} title="Delete job card"><Trash2 size={15} /></IconBtn>
                    )
                  )}
                </div>
              </div>
          ...
          {form.partsUsed.map((pu, idx) => {
            ...
            return (
              <div key={idx} style={{ display: "flex", alignItems: "center", gap: 8, background: "#15171A", borderRadius: 6, padding: "8px 10px" }}>
                ...
                {isOwner && <div style={{ fontSize: 12, color: "#9A9D9F", whiteSpace: "nowrap" }}>{part ? money(part.cost * pu.qty) : ""}</div>}
              ...
              return (
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#9A9D9F" }}>{p.sku || ""}{p.sku && p.supplier ? " · " : ""}{p.supplier || ""}</div>
                  </div>
                  {isOwner && <div style={{ fontSize: 12, color: "#9A9D9F", whiteSpace: "nowrap" }}>{money(p.cost)}</div>}
                  <div style={{
                ...
                {job.partsUsed.map((pu, i) => {
                  ...
                  return (
                    <tr key={i} style={{ borderBottom: "1px solid #eee" }}>
                      ...
                      {showCost && <td style={{ padding: "4px 0", textAlign: "right", width: 70 }}>{part ? money(part.cost * pu.qty) : "—"}</td>}
                    </tr>
                  );
                })}
              </tbody>
...
function PartForm({ part, onSave, onClose }) {
  const [form, setForm] = useState(part ? { ...part } : { id: uid("part"), name: "", sku: "", cost: 0, qty: 0, reorderAt: 0, supplier: "", category: "Consumables" });
  const [formError, setFormError] = useState("");
  return (
    <Modal onClose={onClose} title={part ? "Edit item" : "New part / consumable"} wide>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 14, marginBottom: 14 }}>
        <div><label>Name</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. MIG Wire 0.8mm (5kg spool)" /></div>
        <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 18 }}>
        ...
        <div><label>Qty in stock</label><input type="number" min="0" value={form.qty} onChange={(e) => setForm({ ...form, qty: e.target.value })} /></div>
        <div><label>Reorder at</label><input type="number" min="0" value={form.reorderAt} onChange={(e) => setForm({ ...form, reorderAt: e.target.value })} /></div>
      </div>
      {formError && <div style={{ color: "#D9695A", fontSize: 12.5, marginBottom: 10 }}>{formError}</div>}
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
        <button className="wj-btn wj-ghost" style={{ padding: "10px 18px", borderRadius: 6, fontSize: 13.5, fontWeight: 600 }} onClick={onClose}>Cancel</button>
        <button className="wj-btn wj-amber" style={{ padding: "10px 20px", borderRadius: 6, fontSize: 13.5, fontWeight: 700 }} onClick={() => {
          ...
          onSave({ ...form, cost: Number(form.cost), qty: Number(form.qty), reorderAt: Number(form.reorderAt), category: cat });
        }}>Save item</button>
      </div>
    </Modal>
  );
      ...
      {filtered.length === 0 ? <Empty text="No inventory items match." /> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 12, padding: "0 14px", fontSize: 11, color: "#5C6065", textTransform: "uppercase", letterSpacing: "0.04em" }}>
            ...
            <div style={{ flex: 1 }}>SKU</div>
            <div style={{ flex: 1 }}>Supplier</div>
            {isOwner && <div style={{ flex: 1 }}>Unit cost</div>}
            {isOwner && <div style={{ flex: 1 }}>Stock value</div>}
            <div style={{ flex: 1 }}>Used on jobs</div>
            <div style={{ flex: "0 0 120px", textAlign: "center" }}>Qty</div>
          ...
          {filtered.map((p) => {
            ...
            return (
              <div key={p.id} className="wj-card" style={{ padding: "12px 14px", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                <div style={{ flex: 3, minWidth: 120 }}>
                  <div style={{ fontWeight: 600, fontSize: 13.5 }}>{p.name}</div>
                </div>
                <div style={{ flex: 1, fontSize: 12.5, color: "#9A9D9F" }}>{p.sku || "—"}</div>
                <div style={{ flex: 1, fontSize: 12.5, color: "#9A9D9F" }}>{p.supplier || "—"}</div>
                {isOwner && <div style={{ flex: 1, fontSize: 12.5 }}>{money(p.cost)}</div>}
                ...
                <div style={{ flex: "0 0 120px", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                  <button className="wj-btn wj-ghost" style={{ width: 26, height: 26, borderRadius: 5, fontSize: 15 }} onClick={() => adjustQty(p.id, -1)}>−</button>
                  <span style={{ fontWeight: 700, fontSize: 14, minWidth: 24, textAlign: "center", color: Number(p.qty) === 0 ? "#D9695A" : Number(p.qty) <= Number(p.reorderAt || 0) ? "#C9760C" : "#E8E6DF" }}>{p.qty}</span>
                  <button className="wj-btn wj-ghost" style={{ width: 26, height: 26, borderRadius: 5, fontSize: 15 }} onClick={() => adjustQty(p.id, 1)}>+</button>
                </div>
                <div style={{ flex: 1 }}>
                ...
                {isOwner && (
                  <div style={{ width: 90, display: "flex", gap: 4, justifyContent: "flex-end", alignItems: "center" }}>
...
function Modal({ children, onClose, title, wide }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 90, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div className="wj-card" style={{ width: wide ? 560 : 440, maxHeight: "88vh", overflow: "auto", padding: 22 }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <div className="wj-h" style={{ fontSize: 17, fontWeight: 700 }}>{title}</div>
          <button className="wj-btn" style={{ background: "transparent", color: "#9A9D9F" }} onClick={onClose}><X size={18} /></button>
        </div>
        {children}
      </div>
    </div>
