...
function AuthGate() {
  ...
  const handleSetup = async () => {
    ...
    try {
      ...
      // Set the session immediately so the profile insert is authenticated
      if (data.session) {
      ...
      // If no session yet (email confirmation required), tell user
      if (!data.session) {
        setError("Account created! Check your email to confirm, then sign in.");
        setLoading(false);
        return;
      }
      // Session is set — onAuthStateChange will fire and log them in automatically
    } catch (e) {
