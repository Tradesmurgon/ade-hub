...
function AuthGate() {
  ...
  // Controlled inputs for setup form
  ...
  const handleSetup = async () => {
    ...
    try {
      ...
      // Use RPC function with security definer to bypass RLS for initial owner creation
      ...
      // onAuthStateChange will fire automatically and load the app
    } catch (e) {
...
function StaffAccess({ staffUsers, persistStaff, session, jobs, persistJobs }) {
  ...
  const apiCall = async (method, body) => {
    // Always get a fresh token — stored token can expire after 1 hour
    const { data: { session: currentSession } } = await supabase.auth.getSession();
    const token = currentSession?.access_token;
    if (!token) { return { error: "No active session — please sign in again." }; }
