...
/* ============================================================
   ...
   ============================================================ */
function Dashboard({ jobs, clients, clientById, jobCost, setTab, company, saveCompany, goToJob }) {
      ...
      {active.length === 0 ? <Empty text="No active jobs." /> : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 12 }}>
          {active.slice(0, 9).map((j) => {
            ...
            return (
              <div key={j.id} className="wj-card" style={{ padding: 14, cursor: "pointer", transition: "border-color 0.15s" }}
                onClick={() => goToJob(j.id)}
                onMouseEnter={(e) => e.currentTarget.style.borderColor = "#FF8A1E"}
              ...
              >
                ...
                <div style={{ fontSize: 12.5, color: "#9A9D9F", marginBottom: 6 }}>{clientById[j.clientId]?.name || "No client"}</div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div style={{ fontSize: 12, color: "#FF8A1E", fontWeight: 600, marginLeft: "auto" }}>{money(cost.total)}</div>
                </div>
                <div style={{ fontSize: 11, color: "#5C6065", marginTop: 6, textAlign: "right" }}>Tap to open →</div>
              </div>
            );
          })}
        </div>
...
function Jobs({ isOwner, can, session, jobs, clients, parts, clientById, partById, persistJobs, jobCost, setPrintJob, staffUsers, openJobId, clearOpenJobId, onRequestComplete }) {
  ...
  // Auto-open a specific job when navigated from dashboard
  useEffect(() => {
    if (openJobId) {
      const job = jobs.find((j) => j.id === openJobId);
      if (job) { setEditing(job); setShowForm(true); }
      if (clearOpenJobId) clearOpenJobId();
    }
  }, [openJobId]);
