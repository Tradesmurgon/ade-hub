...
export default async function handler(req, res) {
  ...
  // ── CREATE staff ──────────────────────────────────────────
  if (req.method === 'POST') {
    const { name, email, password, role, permissions, rate } = req.body
  ...
  // ── UPDATE staff ──────────────────────────────────────────
  if (req.method === 'PUT') {
    const { id, name, role, permissions, password, rate } = req.body
    ...
    const { error: profileErr } = await adminSupabase
      .from('profiles').update({ name, role, permissions, rate: Number(rate) || 0 }).eq('id', id)
